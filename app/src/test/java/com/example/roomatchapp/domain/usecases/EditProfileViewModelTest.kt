...
class EditProfileViewModelTest {
    ...
    fun setUp() = runTest {
        ...
        `when`(userRepository.getRoommate("123")).thenReturn(mockRoommate)
        `when`(userRepository.updateRoommate(eq("123"), any(Roommate::class.java))).thenReturn(true)
    ...
    fun `roommate remains null when loading fails`() = runTest {
        `when`(userRepository.getRoommate("badId")).thenThrow(RuntimeException("fail"))
    ...
    fun `saveChanges does not update uiState if repository update fails`() = runTest {
        `when`(userRepository.updateRoommate(eq("123"), any())).thenReturn(false)
        viewModel.saveChanges(
    ...
    fun `saveChanges handles failure gracefully`() = runTest(testDispatcher) {
        `when`(userRepository.updateRoommate(eq("123"), any(Roommate::class.java))).thenReturn(false)
